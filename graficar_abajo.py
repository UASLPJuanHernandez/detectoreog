"""
Grafica las adquisiciones de ABAJO.csv con navegación individual.
Cada adquisición tiene su número → dile a Claude cuál borrar.
Uso: python3 graficar_abajo.py [archivo.csv]
"""
...
if len(sys.argv) > 1:
    ...
    ruta = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ABAJO.csv")
...
if not señal:
    print("No hay filas con etiqueta 'abajo'.")
    sys.exit(1)
...
fig1.suptitle(
    f"Abajo — {os.path.basename(archivo)}  ({n_muestras} adquisiciones, {n_pts} pts)",
...
plt.savefig(os.path.join(os.path.dirname(os.path.abspath(archivo)),
            f"abajo_{os.path.splitext(os.path.basename(archivo))[0]}.png"),
...
fig2.canvas.manager.set_window_title("Adquisiciones individuales — Abajo")
