"""
Interfaz EOG con fusión de movimientos diagonales (Opción C — secuencial).
Requiere: modelo_eog.pkl  y  encoder_eog.pkl  (correr entrenar_eog.py primero)
Uso: python3 interfaz_eog_optimizado_diagonal.py
"""
...
# ── Ventana de fusión diagonal ────────────────────────────────────
# Tras detectar el primer movimiento, el sistema espera este tiempo
# para recibir un segundo movimiento ortogonal y formar un diagonal.
# Si no llega ninguno, emite el movimiento simple.
FUSION_WINDOW     = 1.20   # segundos
UMBRAL_FUSION_LAT = 600    # amplitud mínima para detectar lateral en fusión post-vertical
...
# Comandos Arduino:  L R U D C  (simples)  +  Q E Z X  (diagonales)
COMANDOS = {
    "izquierda":        b'L',
    "derecha":          b'R',
    "reposo":           b'C',
...
# Orden de clases en el panel de conteos
_CLASES_UI = (
...
class AppEOG:
    ...
    def _construir_ui(self):
        ...
        col_res = tk.Frame(centro, bg=BG2, width=270)
        ...
        self.lbl_flecha = tk.Label(col_res, text="–", bg=BG2,
                                    fg=COLORES["–"], font=("Helvetica", 72, "bold"))
        self.lbl_flecha.pack()

        self.lbl_dir = tk.Label(col_res, text="–", bg=BG2,
                                 fg=TEXT, font=("Helvetica", 15, "bold"))
        self.lbl_dir.pack(pady=(0, 4))

        self.lbl_conf = tk.Label(col_res, text="confianza: –", bg=BG2,
                                  fg=DIM, font=("Helvetica", 10))
        self.lbl_conf.pack()

        tk.Frame(col_res, bg=DIM, height=1).pack(fill="x", padx=16, pady=10)
        ...
        for clase in _CLASES_UI:
            ...
            tk.Label(f, text=f"{FLECHAS[clase]} {clase}", bg=BG2,
                     fg=COLORES[clase], font=("Helvetica", 9)).pack(side="left")
        ...
        self.lbl_instruccion = tk.Label(
            ...
            padx=20, pady=12)
        self.lbl_instruccion.place(relx=0.42, rely=0.42, anchor="center")
        ...
        self.lbl_ultimo = tk.Label(
            ...
            wraplength=680, justify="center",
            padx=20, pady=6)
        self.lbl_ultimo.place(relx=0.42, rely=0.56, anchor="center")
    ...
    def _hilo_lectura(self):
        self._instruccion("listo",
            "Mueve el ojo   → ← ↑ ↓  o diagonal ↗ ↖ ↘ ↙", SUCCESS)
        while self.leyendo and self.ser:
            ...
            # Onset detection suspendida durante fusión: la línea base inflada
            # del retorno de "arriba" bloquearía señales laterales más pequeñas.
            # buf_cls acumula libremente; si el primer round vota "arriba" de
            # nuevo se ignora y el siguiente round ya tiene señal limpia.
            if not self._fusion_pending:
                if self.base_activa:
    ...
    def _votar(self):
        # Ruta especial: fusión con primer movimiento vertical.
        # El modelo ML falla aquí porque la señal está mezclada con el
        # retorno del movimiento vertical — usar heurística de amplitud.
        if self._fusion_pending and self._fusion_primera in ("arriba", "abajo"):
            self._votar_lateral_fusion()
            return
        ...
        # Durante fusión no bloqueamos opuestos por votos: el retorno del primer
        # movimiento puede meter votos del lado contrario y bloquear el segundo
        if not self._fusion_pending:
            opuesta_voto = OPUESTAS.get(voto)
            if opuesta_voto and self.votos.count(opuesta_voto) > 0:
                voto = "reposo"

        # Durante la ventana de fusión se permite vertical después de horizontal
        if not self._fusion_pending:
        ...
        fusion_tag = f" [fusión:{self._fusion_primera}]" if self._fusion_pending else ""
        resumen = (f"votos: {list(self.votos)}  conf={confianza:.0%} "
                   f"amp={amp_abs_rt:.0f}{bloq}{fusion_tag}")
    ...
    # ── Lógica de fusión diagonal ─────────────────────────────────

    def _votar_lateral_fusion(self):
        """Detecta el segundo movimiento horizontal (izq/der) cuando el primero
        fue vertical. Usa la amplitud del tramo final de buf_cls en lugar del
        modelo ML, que falla con señal mezclada con el retorno vertical."""
        if len(self.buf_cls) < VENTANA_CLS:
            return

        seg       = np.array(list(self.buf_cls), dtype=float)
        tail_mean = seg[-25:].mean()   # últimos ~125 ms

        if tail_mean > UMBRAL_FUSION_LAT:
            direccion = "derecha"
        elif tail_mean < -UMBRAL_FUSION_LAT:
            direccion = "izquierda"
        else:
            self.q.put(("log", f"fusión_lat: tail={tail_mean:.0f} — esperando señal clara"))
            return

        confianza = min(0.50 + abs(tail_mean) / 4000, 0.90)
        self.votos.append(direccion)

        ahora = time.time()
        self.q.put(("log", f"fusión_lat: {list(self.votos)}  tail={tail_mean:.0f}"))
    ...
    def _disparar(self, direccion, confianza):
        """Punto de entrada de cada movimiento detectado.
        Si hay ventana de fusión activa, intenta combinar.
        Si no, abre la ventana y espera el segundo movimiento.
        """
        ahora = time.time()

        # ── Segundo movimiento dentro de la ventana ───────────────
        if self._fusion_pending and ahora < self._fusion_fin:
            diagonal = _DIAGONALES.get((self._fusion_primera, direccion))
            if diagonal:
                # Combinación válida → emitir diagonal
                if self._fusion_timer:
                    self._fusion_timer.cancel()
                    self._fusion_timer = None
                self._fusion_pending = False
                conf_final = max(confianza, self._fusion_conf)
                self._enviar(diagonal, conf_final)
                return
            # Movimiento incompatible (mismo eje o reposo) → ignorar,
            # dejar que el timer emita el movimiento original.
            return

        # ── Primer movimiento: abrir ventana de fusión ────────────
        self._fusion_pending = True
        self._fusion_primera = direccion
        self._fusion_conf    = confianza
        self._fusion_fin     = ahora + FUSION_WINDOW

        # Pizarra limpia para el segundo movimiento:
        # - resetea buf_cls para que el modelo vea señal fresca
        # - limpia base_buf para que el umbral de onset no quede inflado
        #   por la señal grande de "arriba" (su retorno al centro tiene alta std)
        # - reactiva onset detection inmediatamente (no esperar post_onset)
        # - limpia votos para que la regla HORIZONTALES no bloquee verticales
        self.buf_cls.clear()
        self.base_buf.clear()
        self.n_nuevos       = 0
        self.votos.clear()
    ...
    def _fusion_fallback(self, direccion, confianza):
        """Llamado por el timer si no llega un segundo movimiento."""
    ...
    def _enviar(self, direccion, confianza):
        """Emite el comando final (simple o diagonal) al Arduino y actualiza la UI."""
        ...
        # Resetea línea base para que el período de reposo post-comando
        # construya una referencia limpia — evita drift acumulado de señales
        # grandes como "arriba" que sesgan base_mean en siguientes detecciones
        self.base_buf.clear()
        self.base_activa    = True
        self.post_onset_cnt = 0
        self.consec_sobre   = 0
        ...
        self.q.put(("ultimo",
            f"Último:  {FLECHAS.get(direccion,'–')} {direccion.upper()}  {confianza:.0%} ✓",
            color))
        self._estado_inst = None
