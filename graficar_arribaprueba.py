"""
Grafica las adquisiciones de ARRIBAPRUEBA.csv con navegación individual.
Cada adquisición tiene su número → dile a Claude cuál borrar.
Uso: python3 graficar_arribaprueba.py [archivo.csv]
"""
...
if len(sys.argv) > 1:
    ...
    ruta = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ARRIBAPRUEBA.csv")
...
if not señal:
    print("No hay filas con etiqueta 'arriba'.")
    sys.exit(1)
...
fig1.suptitle(
    f"Arriba Prueba — {os.path.basename(archivo)}  ({n_muestras} adquisiciones, {n_pts} pts)",
...
_ruta_png = os.path.join(os.path.dirname(os.path.abspath(archivo)),
            f"arribaprueba_{os.path.splitext(os.path.basename(archivo))[0]}.png")
...
fig2.canvas.manager.set_window_title("Adquisiciones individuales — Arriba Prueba")
