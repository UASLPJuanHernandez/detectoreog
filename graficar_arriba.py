"""
Grafica las adquisiciones de ARRIBA.csv con navegación individual.
Cada adquisición tiene su número → dile a Claude cuál borrar.
Uso: python3 graficar_arriba.py [archivo.csv]
"""
...
if len(sys.argv) > 1:
    ...
    ruta = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ARRIBA.csv")
...
fig1.suptitle(
    f"Arriba — {os.path.basename(archivo)}  ({n_muestras} adquisiciones, {n_pts} pts)",
...
fig2.canvas.manager.set_window_title("Adquisiciones individuales — Arriba")
